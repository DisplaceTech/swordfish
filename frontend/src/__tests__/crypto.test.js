...
describe('deriveVerifier', () => {
  it('returns a 64-char hex string (32 bytes)', async () => {
    const verifier = await deriveVerifier('my passphrase')
    expect(verifier).toHaveLength(64)
    expect(HEX_RE.test(verifier)).toBe(true)
  })

  it('returns the same verifier for the same passphrase', async () => {
    const a = await deriveVerifier('passphrase')
    const b = await deriveVerifier('passphrase')
    expect(a).toBe(b)
  })

  it('returns different verifiers for different passphrases', async () => {
    const a = await deriveVerifier('passphrase1')
    const b = await deriveVerifier('passphrase2')
    expect(a).not.toBe(b)
  })

  it('matches the verifier embedded in the encrypt() creation string', async () => {
    const passphrase = 'test-passphrase'
    const [, verifierFromEncrypt] = (await encrypt('secret', passphrase)).split('$')
    const verifierDirect = await deriveVerifier(passphrase)
    expect(verifierDirect).toBe(verifierFromEncrypt)
  })
})
